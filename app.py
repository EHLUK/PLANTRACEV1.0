...
# -----------------------------------------------------------------------------
# -----------------------------------------------------------------------------
# CUSTOM CSS  --  PlanTrace v4  -- Premium Project Controls UI
# Palette: Navy #07111F | Ink #0F1E2E | Amber #E8951D | Teal #1AADAA | Red #C84040 | Green #1E7E52
# Typeface: IBM Plex Sans (sharp, technical, authoritative)
# -----------------------------------------------------------------------------
st.markdown("""
<style>
...
[data-testid="stAppViewContainer"]{background:#EFF1F4!important}
...
/* ================================================================
   SIDEBAR  — dark, tight, architectural
================================================================ */
[data-testid="stSidebar"]{
    background:#07111F!important;
    border-right:1px solid #0c1c2e;
...
/* ================================================================
   CONTROL BAR  — the header strip above page content
================================================================ */
.ctrl-bar{
    background:#07111F;
...
.ctrl-bar-desc{
    font-size:11.5px;color:#3D5470;margin-top:4px;font-weight:400}
.ctrl-bar-meta{display:flex;gap:20px;align-items:center;flex-wrap:wrap}
...
.ctrl-meta-value{
    font-size:13px;font-weight:600;color:#4A6278;margin-top:1px;
...
.stTabs [data-baseweb="tab"]{
    font-family:'IBM Plex Sans',sans-serif!important;
    font-size:12.5px;font-weight:600;color:#5A7388;
    padding:11px 20px;
    border-bottom:2px solid transparent;
    margin-bottom:-1px;
    background:transparent;border-radius:0;letter-spacing:0.1px}
.stTabs [aria-selected="true"]{
    color:#07111F!important;
    border-bottom:2px solid #E8951D!important;
    background:transparent!important}
.stTabs [data-baseweb="tab"]:hover{
    color:#07111F!important;background:#F5F7F9!important}
...
.kpi{
    ...
    border-radius:6px;
    padding:16px 18px;
    box-shadow:0 1px 3px rgba(7,17,31,0.06);
    position:relative;
    overflow:hidden}
.kpi::after{
    content:'';
    ...
    height:3px}
.kpi-border-top-navy::after  {background:#07111F}
.kpi-border-top-red::after   {background:#C84040}
.kpi-border-top-amber::after {background:#E8951D}
.kpi-border-top-green::after {background:#1E7E52}
.kpi-border-top-blue::after  {background:#1AADAA}
.kpi-label{
    font-size:10px;font-weight:700;color:#8A9BB0;
    letter-spacing:1.2px;text-transform:uppercase;
    margin-bottom:8px;font-family:'IBM Plex Mono',monospace}
.kpi-num{
    font-size:32px;font-weight:700;color:#07111F;
    line-height:1;margin-bottom:4px;
    font-family:'IBM Plex Mono',monospace;letter-spacing:-1px}
.kpi-sub{font-size:11px;color:#7A8FA2}
.kpi-red   .kpi-num{color:#C84040}
.kpi-amber .kpi-num{color:#C07818}
.kpi-green .kpi-num{color:#1E7E52}
.kpi-blue  .kpi-num{color:#1AADAA}
...
.chip-red   {background:#FAEAEA;color:#9E2828;border:1px solid #F0BABA}
.chip-amber {background:#FDF3E3;color:#9A6010;border:1px solid #F0D090}
.chip-green {background:#E5F5EC;color:#155F3A;border:1px solid #A8DCC0}
.chip-blue  {background:#E3F5F5;color:#0F7878;border:1px solid #90D8D8}
.chip-grey  {background:#F0F2F5;color:#4A5A68;border:1px solid #D0D8E0}
.chip-navy  {background:#0a1929;color:#4A6278;border:1px solid #0f2538}
...
.stDataFrame{
    ...
    border:1px solid #DDE2E8;
    overflow:hidden;
    box-shadow:0 1px 3px rgba(7,17,31,0.05)}
...
.stDataFrame tbody tr td{
    ...
    font-family:'IBM Plex Sans',sans-serif!important}
.stDataFrame tbody tr:nth-child(even){background:#F7F9FA}
.stDataFrame tbody tr:hover{background:#F0F4F7}
div[data-testid="metric-container"]{
    background:#FFFFFF;border-radius:6px;
    padding:16px;border:1px solid #DDE2E8;
    box-shadow:0 1px 3px rgba(7,17,31,0.06)}
...
[data-testid="stAppViewContainer"] .stButton>button{
    ...
    box-shadow:0 1px 3px rgba(7,17,31,0.2)}
[data-testid="stAppViewContainer"] .stButton>button:hover{
    background:#0F1E2E;
    box-shadow:0 3px 12px rgba(7,17,31,0.3);
    transform:translateY(-1px)}
[data-testid="stAppViewContainer"] .stButton>button[kind="primary"]{
    background:#E8951D;color:#07111F}
[data-testid="stAppViewContainer"] .stButton>button[kind="primary"]:hover{
    background:#d4841a}
.stDownloadButton>button{
    background:#FFFFFF!important;color:#07111F!important;
    border:1.5px solid #07111F!important;
    border-radius:4px!important;font-weight:600!important;
    font-family:'IBM Plex Sans',sans-serif!important;
    font-size:12.5px!important;padding:8px 18px!important;
    transition:all 0.12s ease!important}
.stDownloadButton>button:hover{
    background:#07111F!important;color:#FFFFFF!important;
...
.stSelectbox>div>div,.stMultiSelect>div>div{
    border-radius:4px;border:1px solid #C8D4DE;
    font-size:13px;background:#FFFFFF;color:#1a2332;
    font-family:'IBM Plex Sans',sans-serif}
...
.stDateInput>div>div>input{
    border-radius:4px;border:1px solid #C8D4DE;font-size:13px}
...
.streamlit-expanderHeader{
    ...
    color:#1a2332;padding:10px 14px}
...
h2{
    ...
    color:#07111F!important;margin:20px 0 10px 0!important;
    letter-spacing:-0.2px}
h3{
    font-family:'IBM Plex Sans',sans-serif!important;
    font-size:14px!important;font-weight:600!important;
    color:#1a2332!important;margin:14px 0 8px 0!important}
h4,h5{
    font-family:'IBM Plex Sans',sans-serif!important;
...
.stCaption,.stCaption p{
    font-size:11.5px!important;color:#8A9BB0!important;
    font-family:'IBM Plex Sans',sans-serif!important}
...
================================================================ */
.section-label{
    font-size:10px;font-weight:700;color:#7A8FA2;
...
.card{
    ...
    padding:20px;box-shadow:0 1px 3px rgba(7,17,31,0.06)}
.card-flat{
    background:#FFFFFF;border:1px solid #DDE2E8;border-radius:6px;padding:16px}
.panel{
    background:#F7F9FA;border:1px solid #DDE2E8;border-radius:6px;padding:16px}
/* Attention items */
.attn{
    background:#FFFFFF;
    border:1px solid #DDE2E8;
...
.attn-blue {border-left-color:#1AADAA}
/* Empty state */
.empty-state{
    background:#FFFFFF;border:2px dashed #C8D4DE;border-radius:6px;
    padding:48px 24px;text-align:center}
/* Data quality rows */
.dq-row{
    display:flex;align-items:center;justify-content:space-between;
    padding:7px 0;border-bottom:1px solid #EEF0F3;font-size:12px}
.dq-row:last-child{border-bottom:none}
/* Status dots */
.dot-green{display:inline-block;width:6px;height:6px;border-radius:50%;
           background:#1E7E52;margin-right:5px;vertical-align:middle}
...
.dot-red  {display:inline-block;width:6px;height:6px;border-radius:50%;
           background:#C84040;margin-right:5px;vertical-align:middle}
/* Page header card */
.pt-page-header{
    background:linear-gradient(135deg,#07111F 0%,#0F1E2E 100%);
...
def page_milestone_tracker(data: dict, near_crit_days: float):
    ...
    for tab_idx, (_, ms) in enumerate(milestones.head(10).iterrows(), start=1):
        ...
        with tabs[tab_idx]:
            ...
            # ---- Action buttons ---------------------------------------------
            ba, bb = st.columns(2)
            btn_drive = ba.button("Find Driving Path",  key=f"ms_drive_{ms_id}", width="stretch")
            btn_succ  = bb.button("Show Successors",    key=f"ms_succ_{ms_id}",  width="stretch")
...
def _sidebar() -> tuple:
    with st.sidebar:
        ...
        # -- Brand -------------------------------------------------------------
        logo_html = _logo_b64(width=68)
        st.markdown(
            f'<div style="padding:18px 16px 14px;border-bottom:1px solid #0c1c2e;">'
            f'<div style="display:flex;align-items:center;gap:11px;">'
            f'{logo_html}'
            f'<div>'
            f'<div style="font-size:15px;font-weight:700;color:#FFFFFF;'
            f'letter-spacing:-0.2px;line-height:1.15;'
            f'font-family:\'IBM Plex Sans\',sans-serif;">PlanTrace</div>'
        ...
        st.markdown(
            ...
            '<div style="font-size:10px;color:#253A50;font-weight:700;'
            'letter-spacing:1px;text-transform:uppercase;margin-bottom:6px;'
            'font-family:\'IBM Plex Mono\',monospace;">Programme</div>',
            unsafe_allow_html=True,
        )
        ...
        if "programme" in st.session_state:
            ...
            pname_html = (
                f'<div style="font-size:10px;color:#2D4055;margin-bottom:2px;'
                f'white-space:nowrap;overflow:hidden;text-overflow:ellipsis;"'
                f' title="{pname}">{pname}</div>'
                if pname else ""
            )
            st.markdown(
                f'<div style="margin:0 12px 12px;background:#0a1929;'
                f'border:1px solid #0f2538;border-radius:5px;padding:10px 12px;">'
                f'<div style="display:flex;align-items:center;gap:5px;margin-bottom:6px;">'
                f'<div style="width:5px;height:5px;border-radius:50%;'
                f'background:#1E7E52;flex-shrink:0;"></div>'
                f'<span style="font-size:9px;font-weight:700;color:#1E7E52;'
                f'text-transform:uppercase;letter-spacing:1px;'
                f'font-family:\'IBM Plex Mono\',monospace;">Loaded</span>'
                f'</div>'
                f'<div style="font-size:11px;font-weight:600;color:#8CAABF;'
                f'white-space:nowrap;overflow:hidden;text-overflow:ellipsis;'
                f'margin-bottom:2px;" title="{fname}">{fname}</div>'
                f'{pname_html}'
                f'<div style="display:grid;grid-template-columns:1fr 1fr;'
                f'gap:4px;margin-top:8px;">'
                f'<div style="background:#060e1a;border-radius:3px;padding:5px 8px;">'
                f'<div style="font-size:9px;color:#253A50;text-transform:uppercase;'
                f'letter-spacing:0.8px;font-family:\'IBM Plex Mono\',monospace;'
                f'margin-bottom:2px;">Activities</div>'
                f'<div style="font-size:15px;font-weight:700;color:{_AMBER};'
                ...
                f'<div style="font-size:9px;color:#253A50;text-transform:uppercase;'
                f'letter-spacing:0.8px;font-family:\'IBM Plex Mono\',monospace;'
                f'margin-bottom:2px;">Rels</div>'
                f'<div style="font-size:15px;font-weight:700;color:{_AMBER};'
                f'font-family:\'IBM Plex Mono\',monospace;letter-spacing:-0.5px;">'
                f'{nrels:,}</div>'
                f'</div></div>'
                f'<div style="font-size:10px;color:#253A50;margin-top:7px;'
                f'font-family:\'IBM Plex Mono\',monospace;">'
                f'Data date <span style="color:#3D5470;">{dd_s}</span></div>'
                f'</div>',
                unsafe_allow_html=True,
            )
        else:
            st.markdown(
                ...
                f'<span style="font-size:9px;font-weight:700;color:#253A50;'
                f'text-transform:uppercase;letter-spacing:1px;'
                f'font-family:\'IBM Plex Mono\',monospace;">No Programme</span>'
                f'</div>'
                f'<div style="font-size:11px;color:#1a2d40;margin-top:6px;'
        ...
        st.markdown(
            ...
            '<div style="font-size:10px;color:#1e3550;font-weight:700;'
            'letter-spacing:1px;text-transform:uppercase;margin-bottom:6px;'
        ...
        for key, label in _NAV:
            ...
            if is_active:
                div_style = (
                    f'background:#0a1929;'
                    f'border-left:2px solid {_AMBER};'
                    f'border-radius:0;margin:0;'
                )
            elif is_disabled:
                div_style = "margin:0;opacity:0.25;"
            else:
                div_style = "margin:0;"

            st.markdown(f'<div style="{div_style}">', unsafe_allow_html=True)
        ...
        for _m in ["PM Mode", "Planner Mode"]:
            _a  = (_cur == _m)
            _bd = f"border-left:2px solid {_AMBER};" if _a else "border-left:2px solid transparent;"
            _bg = "#0a1929" if _a else "transparent"
            st.markdown(
                f'<div style="margin:0;{_bd}background:{_bg};">',
                unsafe_allow_html=True,
            )
            if st.button(_m, key=f"mode_{_m.replace(' ','_')}",
                         use_container_width=True):
        ...
        # -- Footer ------------------------------------------------------------
        st.markdown(
            '<div style="padding:14px 16px;border-top:1px solid #0c1c2e;margin-top:12px;">'
            '<div style="font-size:10px;color:#1a2d40;line-height:1.8;'
            'font-family:\'IBM Plex Mono\',monospace;">'
            'P6 &rarr; File &rarr; Export &rarr; P6 XER'
            '</div></div>',
            unsafe_allow_html=True,
        )
